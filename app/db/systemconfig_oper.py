...
class SystemConfigOper(DbOper, metaclass=Singleton):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.__SYSTEMCONF = {}
        for item in SystemConfig.list(self._db):  # noqa
            self.__SYSTEMCONF[item.key] = item.value

    def set(self, key: str | SystemConfigKey, value: Any) -> bool | None:
        """设置系统设置.
        ...
        conf = SystemConfig.get_by_key(self._db, key)  # noqa
        if conf:
            if old_value != value:
                if value:
            ...
            conf.create(self._db)  # noqa
            return True

    async def async_set(self, key: str | SystemConfigKey, value: Any) -> bool | None:
        """异步设置系统设置.
        ...
        # 更新内存(deepcopy避免内存共享)
        self.__SYSTEMCONF[key] = copy.deepcopy(value)
        conf = await SystemConfig.async_get_by_key(self._db, key)  # noqa
        if conf:
            if old_value != value:
                if value:
                    conf.update(self._db, {"value": value})
            ...
            conf = SystemConfig(key=key, value=value)
            await conf.async_create(self._db)  # noqa
            return True
    ...
    def delete(self, key: str | SystemConfigKey) -> bool:
        """删除系统设置."""
        if isinstance(key, SystemConfigKey):
            key = key.value
        # 更新内存
