...
class UserConfigOper(DbOper, metaclass=Singleton):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.__USERCONF = {}
        for item in UserConfig.list(self._db):  # noqa
            self.__set_config_cache(
                username=item.username, key=item.key, value=item.value
            )
    ...
    def set(self, username: str, key: str | UserConfigKey, value: Any):
        ...
        # 写入数据库
        conf = UserConfig.get_by_key(db=self._db, username=username, key=key)  # noqa
        if conf:
            if value:
