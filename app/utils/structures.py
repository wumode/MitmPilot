...
class ListUtils:
    @staticmethod
    def flatten(nested_list: list[list[Any]] | list[Any]) -> list[Any]:
        """将嵌套的列表展平成单个列表.
...
class SetUtils:
    @staticmethod
    def flatten(nested_sets: set[set[Any]] | set[Any]) -> set[Any]:
        """将嵌套的集合展开为单个集合.
        ...
        return {
            item for subset in nested_sets if isinstance(subset, set) for item in subset
        }
